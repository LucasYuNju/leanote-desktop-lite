...
class Application {
  ...
  handleEvents() {
    ...
    ipcMain.on('check-update', (event, arg) => {
      updater.fetchLatestRelease().then((msg) => {
        console.log('new update msg', msg);
        if (msg) {
          event.sender.send('update-found', msg);
        }
      });
    });
    ipcMain.on('patch-update', () => {
      updater.updateToLatestRelease();
    });
  }
