...
class SlateEditor extends Component {
  ...
  onSelectionChange = (selection, state) => {
    ...
    if (isPrevLink || isNextLink || parent.type === INLINES.LINK) {
      if (isPrevLink) {
        ...
        state = state.transform()
          ...
          .apply(OPTIONS);
      } else { // 用户进入一个link元素，将link元素替换成link源码
  ...
  convertSrcToLink = (state) => {
    ...
    const prevText = state.document.getPreviousText(state.startText.key);
    if (prevText && this.lastStartText && prevText.key === this.lastStartText.key && state.startText.text.length === 0) return state;
