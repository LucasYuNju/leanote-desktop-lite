...
User = {
	...
	login: function(username, password, host, callback) {
		...
		db.users.findOne({Username: username, IsLocal: true}, function(err, user) {
			...
			// 本地用户没有, 则远程验证
              ...
              Api.getAvatar(function(avatar) {
                console.error(avatar);
                User.saveCurUserAvatar(avatar, function(res) {
                  // console.error('save status', res);
                });
              });
							callback(true);
						});
  ...
  saveCurUserAvatar: function(avatar, callback) {
    db.users.update({_id: this.userId}, {$set: {Logo: avatar}}, function(err, cnt) {
      if(err || cnt == 0) {
        console.error('save avatar failed', this.UserId, err, cnt);
